            ...
            <table id="myTable">
                ...
                <tbody>
                    ...
                    @foreach($tasks as $key =>$task)
                        ...
                        @endphp
                        @if (!$isDeleted)
                        <tr data-status="{{ $task->task_status }}" style="{{ $task->task_status == 2 ? 'background-color: #c1cfac;' : ($isOverdue ? 'background-color: #f8d7da;' : '') }}">
                            <td>{{$key+1}}</td>
                            ...
                            <td><a href="{{ url('/task-view/'.$task->id) }}">{{ $task->task_owner }}</a></td>
...
<script>
    ...
    $('#downloadButton').on('click', function() {
        ...
        var csvContent = "";
        csvContent += "Number,Sprint,Title,Type,Priority,Project Type,Startdate,Duedate,Status\n";
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            Status = Status.trim();
            csvContent += number + ',"' + Sprint + '","' + title + '","' + type + '","' + priority + '","' + projectType + '","' + Startdate + '","' + Duedate + '","' + Status +'"\n';
        }
...
<script>
    function filterTasks() {
        ...
        rows.forEach(function(row) {
            ...
            if ((selectedStatus === "completed" && status === "2") || 
